...
class Post extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('body', 'required'),
			array('topic_id, forum_id, poster_id, is_edited, edited_by', 'numerical', 'integerOnly'=>true),
			array('poster_ip', 'length', 'max'=>4),
			array('created_at, updated_at', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, body, created_at, updated_at, topic_id, forum_id, poster_id, poster_ip, is_edited, edited_by', 'safe', 'on'=>'search'),
		);
	}
